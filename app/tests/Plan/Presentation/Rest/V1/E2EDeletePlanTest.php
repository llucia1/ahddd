<?php
declare(strict_types=1);

namespace Tests\Plan\Presentation\Rest\V1;

use Faker\Factory as FakerFactory;
use GuzzleHttp\Client;

use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;
use Symfony\Component\HttpFoundation\Response as ResponseCode;

use GuzzleHttp\Handler\MockHandler;
use GuzzleHttp\HandlerStack;
use GuzzleHttp\Psr7\Response;


class E2EDeletePlanTest extends WebTestCase
{
    protected static Client $client;
    protected static $token;
    public static function  setUpBeforeClass(): void
    {

        $mock = new MockHandler([
            new Response(ResponseCode::HTTP_OK, [], json_encode(['token' => 'fake_token'])),
            new Response(ResponseCode::HTTP_OK),
            new Response(ResponseCode::HTTP_NO_CONTENT)
        ]);

        HandlerStack::create($mock);
        self::$client = new Client(['base_uri' => 'http://localhost:80']);


        $response = self::$client->post('/api/v1/auth/login', [
            'json' => [
                'email' => 'xavi@xavi.com',
                'password' => 'password'
            ]
        ]);
        $data = json_decode($response->getBody()->getContents(), true);
        self::$token = $data['token'];
    }
    private static function getPlan() {
        $result = self::$client->get('/api/v1/plan');
        $plan = json_decode($result->getBody()->getContents(),true);
        return $plan[0];
    }

    public  static function testCreatePlanWhenAllDataIsCorrect():void{
        $response = self::$client->request('DELETE', '/api/v1/plan/UuidNotValid',[
            'http_errors' => false, 
            'headers' => [
                'Authorization' => 'Bearer '.self::$token
            ]
        ]);
        self::assertEquals(ResponseCode::HTTP_BAD_REQUEST, $response->getStatusCode());
    }

    public  static function testCreatePlanWhenNotFound():void{
        $faker = FakerFactory::create();
        $response = self::$client->request('DELETE', '/api/v1/plan/' . $faker->uuid(),[
            'http_errors' => false, 
            'headers' => [
                'Authorization' => 'Bearer ' . self::$token
            ]
        ]);
        self::assertEquals(ResponseCode::HTTP_NOT_FOUND, $response->getStatusCode());
    }
    public static function testDeletePlanSuccess():void{

        $plan = self::getPlan();

        $response = self::$client->request('DELETE', '/api/v1/plan/'.$plan['uuid'],[
                                                                                            'http_errors' => false,
                                                                                            'headers' => [
                                                                                                'Authorization' => 'Bearer ' . self::$token
                                                                                            ]
                                                                                        ]);
        self::assertEquals(ResponseCode::HTTP_NO_CONTENT, $response->getStatusCode());
    }
    // php bin/phpunit tests/Plan/Presentation/Rest/V1/E2EDeletePlanTest.php
}
