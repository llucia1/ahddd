...
final class PatchIP4Tag extends AbstractController
{
    public function __construct(private readonly EditIP4Tag $editIP4Tag, private readonly LoggerInterface $logger)
    {
    }
     ...
     */
    #[Patch(
        description: "Edit IP4 Tag with the provided data.",
        summary: "Edit IP4 Tag",
        security: [["Bearer" => []]],
        tags: ["IP4 Tag"],
        responses: [
            "204" => new OAResponse(
    ...
    #[RequestBody(
        ...
        content: new MediaType(
            ...
            schema: new Schema(
                ...
                schema: "EditIP4TagRequest",
                title: "Edit IP4 Tag Request",
                description: "The IP4 Tag data to edit IP4 Tag.",
                required: ["tag"],
                type: "object"
            ),
        ),
    ...
    #[Route('/v1/ip4_tag/{uuid}', name: 'edit_ip4_tag', methods: ['PATCH'])]
    public function __invoke(CreateIP4TagRequest $request, string $uuid): JsonResponse
    {
        try {
            $this->logger->info("Edit IP4 Tag Request", ["request" => $request]);
            $tagUUID = new Ip4TagUuid($uuid);


            if (is_null($request->getTag())) {
                throw new \InvalidArgumentException('Tag not included');
            }

            $tagTag = new Ip4TagTag($request->getTag());
            $ip4Tag = new Ip4Tag($tagUUID, null, $tagTag);

            $this->editIP4Tag->__invoke($ip4Tag);
