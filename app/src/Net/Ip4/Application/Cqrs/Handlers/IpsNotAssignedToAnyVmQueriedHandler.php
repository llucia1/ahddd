...
use GridCP\Net\Ip4\Application\Cqrs\Queries\IpsNotAssignedToAnyVmQueried;
...
use GridCP\Net\Ip4\Application\Response\OnlyIpInArrayResponse;
...
#[AsMessageHandler]
final readonly class IpsNotAssignedToAnyVmQueriedHandler implements QueryHandler
{
    public function __construct(private  readonly  IIp4Repository $ip4Repository)
    {
    }

    public function __invoke(IpsNotAssignedToAnyVmQueried $ips):Ip4sResponse
    {
        try {
            $ipsNotAssignedToVmIp4 = $this->ip4Repository->getIpsNotAssignedToAnyVm( $ips->ips());

            $response = array_map($this->toResponse(), $ipsNotAssignedToVmIp4);
